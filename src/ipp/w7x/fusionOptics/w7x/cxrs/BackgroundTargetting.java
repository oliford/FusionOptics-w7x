...
public class BackgroundTargetting {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for(int iB=0; iB < sys.channelR.length; iB++){
			...
			for(int iP=0; iP < sys.channelR[iB].length; iP++){
				...
				for(Thing thing : Thing.values()){
					ou.outputInfo(System.out, startPoints, hitPoints, beamPlanePos, iB, iP, thing);
				}
		...
		for(int iB=0; iB < sys.channelR.length; iB++){
			for(int iP=0; iP < sys.channelR[iB].length; iP++){		
				ou.outputInfo(jsonOut, startPoints, hitPoints, null, iB, iP, Thing.JSON_LOS);				
			}
		}
		jsonOut.println("]}");
		...
		for(Thing thing : Thing.values()){
			for(int iB=0; iB < sys.channelR.length; iB++){
				for(int iP=0; iP < sys.channelR[iB].length; iP++){			
					ou.outputInfo(System.out, startPoints, hitPoints, null, iB, iP, thing);	
					ou.outputInfo(textOut, startPoints, hitPoints, null, iB, iP, thing);					
				}
			}
		}
		textOut.close();
