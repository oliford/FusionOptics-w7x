...
public class BeamEmissSpecAET21_BottomWindow extends Optic {
	...
	public double entryWindowAngularPosition = -160 * Math.PI / 180; //bottom window
	//public double entryWindowAngularPosition = -40 * Math.PI / 180; // top far window
	public double entryWindowMoveIn = 0.010;
	public double entryWindowThickness = 0.003;
	public double entryWindowCyldLength = 0.026;
	public double entryWindowCyldPosAdjust = 0.018;
	...
	public double backPrismSurfaceAngle = 0 * Math.PI / 180;
	...
	public Disc backPrismFront = new Disc("backPrismFront", backPrismFrontPos, shutterNormal, entryWindowDiameter/2, prismMedium, null, IsoIsoInterface.ideal());
	public Disc backPrismBack = new Disc("backPrismBack", backPrismBackPos, backPrismBackNormal, entryWindowDiameter*0.52, null, prismMedium,  IsoIsoInterface.ideal());
	...
	public double mirrorWidth = 0.040;
	public double mirrorHeight = 0.055;
	
	public double mirrorCentreBackFromWindow = 0.035;	
	public double mirrorCentreAsideFromWindow = 0.028;	
		
	public double mirrorShiftUp = 0.000;
	
	public double mirrorBuildPosA[] = new double[]{ -0.9790062866210938, 6.2445380859375, -0.2304980621337890 };
	public double mirrorNormal[] = Util.reNorm( new double[]{ -10.92113537,  -4.45569688, -13.4474306 } );
	
	public double mirrorCentrePos[] = Util.plus(mirrorBuildPosA, Util.mul(mirrorNormal, -0.007));
		
	//public double mirrorToPortAngle = 85 * Math.PI / 180;
	//public double mirrorTipAngle = 0 * Math.PI / 180;
	
	public double mirrorUp[] = Util.reNorm(Util.cross(Util.cross(portNormal, mirrorNormal),mirrorNormal));
	...
	public double mirror2BackFromMirror1 = 0.025;	
	public double mirror2AsideFromMirror1 = -0.032;

	public double mirror2ToMirror1Angle = 17 * Math.PI / 180;
	public double mirror2TipAngle = 0 * Math.PI / 180;
	
	public double asideVector2[] = Util.reNorm(Algorithms.rotateVector(Algorithms.rotationMatrix(portNormal, 0), asideVector));
	public double mirror2CentrePos[] = Util.plus(Util.plus(mirrorCentrePos, 
														   Util.mul(shutterNormal, mirror2BackFromMirror1)),
														   Util.mul(asideVector2, mirror2AsideFromMirror1));
	
	
	public double mirror2NormalA[] = Util.reNorm(Algorithms.rotateVector(Algorithms.rotationMatrix(mirrorUp, mirror2ToMirror1Angle), asideVector2));
	public double mirror2RightA[] = Util.reNorm(Util.cross(Util.cross(mirror2NormalA, asideVector2), mirror2NormalA));
	public double mirror2Normal[] = Util.reNorm(Algorithms.rotateVector(Algorithms.rotationMatrix(mirror2RightA, mirror2TipAngle), mirror2NormalA));
	
	
	public double mirror2Up[] = Util.reNorm(Util.cross(Util.cross(mirrorUp, mirror2Normal), mirror2Normal));
	...
	//public double lensCentrePos[] = { -0.970, 6.315, -0.150}; //for upper-far window
	public double lensCentrePos[] = { -0.940, 6.315, -0.250}; //for lower window
	public double lensFocalLength = 0.050;
	
	public double lensNormal[] = Util.reNorm(Util.minus(lensCentrePos, mirror2CentrePos));	
	...
	*/
	...
	/*** Fibre plane 1 ****/
	public double fibrePlaneFromLens = 0.050;
	...
	public BeamEmissSpecAET21_BottomWindow() {
