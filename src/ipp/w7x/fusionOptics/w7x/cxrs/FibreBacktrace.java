...
import ipp.w7x.fusionOptics.w7x.cxrs.aek41.BeamEmissSpecAEK41_edgeVIS_ideaOP22;
...
public class FibreBacktrace {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for(int iB=0; iB < sys.channelR.length; iB++){
			...
			for(int iP=0; iP < sys.channelR[iB].length; iP+=1){
				...
				for(Thing j : Thing.values())
					ou.outputInfo(System.out, startPoints, null, beamPlanePos, closestApproachPos, iB, iP, j);
		...
		for(int iB=0; iB < sys.channelR.length; iB++){
			for(int iP=0; iP < sys.channelR[iB].length; iP++){		
				ou.outputInfo(jsonOut, startPoints, null, beamPlanePos, closestApproachPos, iB, iP, Thing.JSON_LOS);				
			}
		}
		jsonOut.println("]}");
		...
		for(int iB=0; iB < sys.channelR.length; iB++){
			for(int iP=0; iP < sys.channelR[iB].length; iP++){		
				ou.outputInfo(losOut, startPoints, null, beamPlanePos, closestApproachPos, iB, iP, Thing.TXT_LOS_MM);				
			}
		}
